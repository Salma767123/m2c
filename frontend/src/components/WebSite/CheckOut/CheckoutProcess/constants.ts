...
export const NAME_REGEX = /^[a-zA-Z\s\-']+$/; // letters, spaces, hyphens, apostrophes

export function formatUSPhone(raw: string): string {
  const cleaned = raw.replace(/\D/g, '');
  // Start with +1 or not, we just want the last 10 digits if it's 11 digits and starts with 1
  let digits = cleaned;
