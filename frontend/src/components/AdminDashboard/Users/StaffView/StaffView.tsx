  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        ...
        <span className="ml-3 text-slate-600">Loading staff details...</span>
      </div>
    );
  }
  ...
  if (!staff) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-lg font-medium text-slate-900">Staff member not found</p>
        <p className="text-sm text-slate-500 mt-1">This account may have been deleted or the link is invalid.</p>
        <Button
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600">Read-only view of staff profile and permissions</p>
          </div>
        </div>
        ...
        <Card className="lg:col-span-1 border border-slate-200">
          <CardContent className="p-6">
            ...
            <div className="flex flex-col items-center text-center">
              <div className="w-20 h-20 bg-brand-500 rounded-full flex items-center justify-center mb-4 overflow-hidden">
                {staff.avatar ? (
                  <img
                    src={staff.avatar}
                    alt={fullName}
            ...
            <div className="mt-6 space-y-4 border-t border-slate-200 pt-6">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm font-medium text-slate-900 break-all">{staff.email}</p>
                </div>
                {staff.isEmailVerified ? (
                  <ShieldCheck className="h-4 w-4 text-green-500 shrink-0 mt-0.5" aria-label="Verified" />
                ) : (
                  <XCircle className="h-4 w-4 text-slate-300 shrink-0 mt-0.5" aria-label="Not verified" />
                )}
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm font-medium text-slate-900">{staff.phone || 'N/A'}</p>
                </div>
          ...
          <Card className="border border-slate-200">
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-3 mb-6">
                <div>
                  <p className="text-xs text-slate-500 mb-1">Assigned Role</p>
            ...
            <Card className="border border-slate-200">
              ...
              <CardContent className="p-6">
                <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg space-y-1">
                  <p className="text-sm text-slate-800">{staff.address.addressLine1}</p>
                  {staff.address.addressLine2 && (
                    <p className="text-sm text-slate-800">{staff.address.addressLine2}</p>
                  )}
                  ...
                  {staff.address.country && staff.address.country !== 'N/A' && (
                    <p className="text-sm text-slate-600">{staff.address.country}</p>
                  )}
                </div>
              </CardContent>
            </Card>
            ...
            <CardContent className="p-6">
              <dl className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm">
                <div>
                  ...
                  <dd className="font-medium text-slate-900 mt-0.5">{fullName}</dd>
                </div>
                <div>
                  <dt className="text-slate-500">Role</dt>
                  <dd className="font-medium text-slate-900 mt-0.5">{staff.role || '—'}</dd>
                </div>
                <div>
