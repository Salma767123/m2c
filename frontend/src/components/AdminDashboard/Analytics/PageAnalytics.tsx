  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl border border-slate-200">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-slate-500 uppercase">Page Views</span>
          </div>
          <p className="text-2xl font-bold text-slate-900">{data.totalPageViews.toLocaleString()}</p>
          {viewsChange !== 0 && (
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-slate-500 uppercase">Unique Visitors</span>
          </div>
          <p className="text-2xl font-bold text-slate-900">{data.uniqueVisitors.toLocaleString()}</p>
        </div>

        <div className="bg-white p-4 rounded-xl border border-slate-200">
          <div className="flex items-center gap-2 mb-2">
            <TrendingDown className="w-4 h-4 text-orange-600" />
            <span className="text-xs font-semibold text-slate-500 uppercase">Bounce Rate</span>
          </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-slate-500 uppercase">Avg Duration</span>
          </div>
          <p className="text-2xl font-bold text-slate-900">{formatDuration(data.avgDuration)}</p>
        </div>
      </div>

      {/* Page Views Trend */}
      <div className="bg-white p-6 rounded-xl border border-slate-200">
        <h3 className="text-sm font-bold text-slate-900 mb-4">Page Views Trend</h3>
        {data.trend.length > 0 ? (
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={data.trend}>
              <defs>
                <linearGradient id="viewsGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#e01a1b" stopOpacity={0.2} />
                  <stop offset="95%" stopColor="#e01a1b" stopOpacity={0} />
                </linearGradient>
              </defs>
              ...
              />
              <Area type="monotone" dataKey="views" stroke="#e01a1b" strokeWidth={2} fill="url(#viewsGradient)" />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50">
                <tr>
                  <th className="text-left px-6 py-3 font-semibold text-slate-600">Page</th>
                  <th className="text-right px-6 py-3 font-semibold text-slate-600">Views</th>
                  <th className="text-right px-6 py-3 font-semibold text-slate-600">Avg Time</th>
                  <th className="px-6 py-3 font-semibold text-slate-600">Traffic</th>
                </tr>
              </thead>
              <tbody>
                {data.topPages.map((page, i) => {
                  ...
                  return (
                    <tr key={page.page} className="border-t border-slate-100 hover:bg-slate-50/50">
                      <td className="px-6 py-3">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-slate-400 w-5">{i + 1}.</span>
                          <span className="font-medium text-slate-900 font-mono text-xs">{page.page}</span>
                        </div>
                      </td>
                      <td className="px-6 py-3 text-right font-medium">{page.views.toLocaleString()}</td>
                      <td className="px-6 py-3 text-right text-slate-600">{formatDuration(page.avgDuration)}</td>
                      <td className="px-6 py-3">
                        <div className="flex items-center gap-2">
            ...
            {data.topPages.length === 0 && (
              <div className="text-center py-8 text-slate-400 text-sm">No page data yet</div>
            )}
          </div>
        </div>
        ...
        {/* Device Breakdown */}
        <div className="bg-white p-6 rounded-xl border border-slate-200">
          <h3 className="text-sm font-bold text-slate-900 mb-4">Device Breakdown</h3>
          {data.deviceBreakdown.length > 0 ? (
                ...
                {data.deviceBreakdown.map((d) => {
                  ...
                  return (
                    <div key={d.device} className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-slate-700 capitalize flex-1">{d.device}</span>
            ...
            <div className="text-center py-8 text-slate-400 text-sm">No device data</div>
          )}
        </div>
      </div>
    </div>
