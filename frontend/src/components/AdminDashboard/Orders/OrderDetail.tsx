...
export default function OrderDetail({ orderId }: OrderDetailProps) {
    ...
    if (isLoading) {
        return <div className="p-6 text-center text-slate-500">Loading order overview...</div>;
    }
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">Order Details Overview</h1>
                        <div className="flex items-center gap-3 mt-1 flex-wrap">
                            <p className="text-sm text-slate-600">Order ID: <span className="font-mono font-semibold">{order.orderId}</span></p>
                            {order.invoiceNo && (
                ...
                <div className="flex gap-3 items-center">
                    ...
                    <span className={`px-4 py-2 rounded-lg font-medium border ${status === "DELIVERED" ? "bg-green-50 text-green-700 border border-green-200 border-green-300" :
                        ["SHIPPED_TO_CUSTOMER", "IN_TRANSIT_TO_ADMIN_HUB"].includes(status) ? "bg-blue-50 text-blue-700 border border-blue-200 border-blue-300" :
                            "bg-yellow-50 text-yellow-700 border border-yellow-200 border-yellow-300"
                        }`}>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
                        <div className="flex items-center gap-2 mb-6">
                            ...
                            <h2 className="text-lg font-semibold text-slate-900">Delivery Status Timeline Snapshot</h2>
                        </div>
                        <div className="space-y-6 relative before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
                            ...
                            <div className="relative pl-8">
                                <div className={`absolute left-0 top-1.5 w-5 h-5 rounded-full border-4 border-white ${["PACKED_BY_VENDOR", "IN_TRANSIT_TO_ADMIN_HUB", "RECEIVED_AT_ADMIN_HUB", "APPROVED_BY_ADMIN_HUB", "SHIPPED_TO_CUSTOMER", "DELIVERED"].includes(status) ? "bg-green-500" : "bg-slate-200"
                                    }`}></div>
                                <div>
                                    <p className="text-sm font-semibold text-slate-900">Packed & Dispatched from Vendor</p>
                                </div>
                            </div>
                            <div className="relative pl-8">
                                <div className={`absolute left-0 top-1.5 w-5 h-5 rounded-full border-4 border-white ${["RECEIVED_AT_ADMIN_HUB", "APPROVED_BY_ADMIN_HUB", "SHIPPED_TO_CUSTOMER", "DELIVERED"].includes(status) ? "bg-green-500" : "bg-slate-200"
                                    }`}></div>
                                <div>
                                    <p className="text-sm font-semibold text-slate-900">Received at Admin Hub</p>
                                </div>
                            </div>
                            <div className="relative pl-8">
                                <div className={`absolute left-0 top-1.5 w-5 h-5 rounded-full border-4 border-white ${status === "DELIVERED" ? "bg-green-500" :
                                    status === "SHIPPED_TO_CUSTOMER" ? "bg-blue-500" : "bg-slate-200"
                                    }`}></div>
                                <div>
                                    <p className="text-sm font-semibold text-slate-900">Out for Final Delivery</p>
                                </div>
                            </div>
                            <div className="relative pl-8">
                                <div className={`absolute left-0 top-1.5 w-5 h-5 rounded-full border-4 border-white ${status === "DELIVERED" ? "bg-green-500" : "bg-slate-200"
                                    }`}></div>
                                <div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h2 className="text-lg font-semibold text-slate-900">Primary Vendor</h2>
                        </div>
                        <div className="space-y-3">
                            <div>
                                <p className="text-xs text-slate-500 uppercase">Store Name</p>
                                <p className="text-sm font-medium text-slate-900">{order.items?.[0]?.vendorName || "Unknown"}</p>
                            </div>
                    ...
                    {order.shipments && order.shipments.length > 0 && (
                        <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
                            <div className="flex items-center gap-2 mb-4">
                                <Truck className="h-5 w-5 text-slate-600" />
                                <h2 className="text-lg font-semibold text-slate-900">Logistics & Shipping</h2>
                            </div>
                            <div className="space-y-4">
                                {order.shipments.map((shipment: any, idx: number) => (
                                    <div key={shipment.id || idx} className="p-4 bg-slate-50 border border-slate-100 rounded-lg space-y-3">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                                                ...
                                                shipment.status === 'SHIPPED_TO_CUSTOMER' ? 'bg-blue-50 text-blue-700 border border-blue-200' :
                                        ...
                                        {shipment.vendorName && (
                                            <div>
                                                ...
                                                <p className="text-sm text-slate-900">{shipment.vendorName}</p>
                                            </div>
                                        )}
                                        {shipment.vendorCarrier && (
                                            <div>
                                                <p className="text-xs text-slate-500 uppercase">Carrier</p>
                                                <p className="text-sm text-slate-900">{shipment.vendorCarrier}</p>
                                            </div>
                                        )}
                                        {shipment.vendorTrackingId && (
                                            <div>
                                                <p className="text-xs text-slate-500 uppercase">Tracking ID</p>
                                                <p className="text-sm font-mono text-slate-900">{shipment.vendorTrackingId}</p>
                                            </div>
                                        )}
                                        {shipment.trackingReference && (
                                            <div>
                                                <p className="text-xs text-slate-500 uppercase">Tracking Reference</p>
                                                <p className="text-sm font-mono text-slate-900">{shipment.trackingReference}</p>
                                            </div>
                                        )}
                                        {shipment.hub?.name && (
                                            <div>
                                                <p className="text-xs text-slate-500 uppercase">Assigned Hub</p>
                                                <p className="text-sm text-slate-900">{shipment.hub.name}</p>
                                            </div>
                                        )}
                                    </div>
                                ))}
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
                        <div className="flex items-center gap-2 mb-4">
                            <HubIcon className="h-5 w-5 text-slate-600" />
                            <h2 className="text-lg font-semibold text-slate-900">Admin Hub Assignment</h2>
                        </div>
                        ...
                        {order.assignedHubId ? (
                            ...
                            <p className="text-sm text-slate-500 italic">No hub assigned yet.</p>
                        )}
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="pt-2 border-t border-slate-50">
                                ...
                                <p className="text-xs font-mono text-slate-900 break-all">{order.paymentId || "N/A"}</p>
                            </div>
                        </div>
                    </div>
                </div>
