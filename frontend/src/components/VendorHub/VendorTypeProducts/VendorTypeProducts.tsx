...
const vendorTypes = [
  {
    id: "manufacturer",
    label: "Manufacturer",
    description: "You produce the goods",
  },
  {
    id: "importer",
    label: "Importer",
    description: "You import goods for resale",
  },
  {
    id: "exporter",
    label: "Exporter",
...
export default function VendorTypeProducts({
  onNext,
  onPrev,
  onUpdateData,
  data,
}: VendorTypeProductsProps) {
  ...
  const handleProductPhotoUpload = (
    categoryId: string,
    productId: string,
  ...
  const updateAdditionalProductName = (
    categoryId: string,
    productId: string,
    name: string,
  ) => {
  ...
  const handleAdditionalProductPhotoUpload = (
    categoryId: string,
    productId: string,
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
  ...
  const handleNext = () => {
    ...
    if (!hasSelectedCategories) {
      ...
      const hasEmptyProducts = Object.keys(formData.selectedCategories).some(
        (catId) => {
    ...
    if (additionalCategories.length > 0) {
      const hasInvalidCustomCategories = additionalCategories.some(
        (c) => !c.name.trim(),
      );
      if (hasInvalidCustomCategories) {
        newErrors.additionalCategories =
          "Please provide a name for all custom categories";
      } else {
        const hasInvalidCustomProducts = additionalCategories.some((c) =>
          c.products.some((p) => !p.name.trim()),
        );
          ...
          <div data-section="marketType">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {marketTypes.map((type) => {
                const isSelected = (formData.marketType || []).includes(
                  type.id,
                );
          ...
          <div
            ...
            className={
              errors.selectedCategories && touched.selectedCategories
                ? "bg-red-50/50 -mx-5 px-5 -mb-6 pb-6 rounded-b-lg"
                : ""
            }
          >
            <div className="space-y-4">
              {loading ? (
                ...
                >
                  <Loader2
                    className="w-5 h-5 animate-spin text-brand-500"
                    aria-hidden="true"
                  />
                ...
                categories.map((category) => {
                  ...
                  return (
                    <div
                      key={category.id}
                      className={`border rounded-lg overflow-hidden transition-colors ${isCategorySelected ? "border-brand-300 bg-brand-50/10" : "border-slate-200 bg-white"}`}
                    >
                      <div className="w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors">
                        <label className="flex items-center gap-3 cursor-pointer flex-1">
                          ...
                          />
                          <span
                            className={`font-semibold text-lg ${isCategorySelected ? "text-brand-900" : "text-slate-900"}`}
                          >
                            {category.name}
                          </span>
                        </label>
                              ...
                              >
                                <div className="flex items-start justify-between gap-4 mb-4">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      removeProduct(category.id, product.id)
                                    }
                                ...
                                <div>
                                  ...
                                  <div className="flex flex-wrap gap-3">
                                    {product.photos.map((photo, photoIndex) => (
                                      <div
                                        key={photoIndex}
                                        className="relative group w-20 h-20 rounded-md overflow-hidden border border-slate-200 bg-white"
                                      >
                                        <Image
                                          src={photo.preview}
                                          alt="Product"
          ...
          <div className="space-y-6">
            ...
            {additionalCategories.length > 0 && (
              <div className="space-y-6">
                {additionalCategories.map((category) => (
                  <div
                    key={category.id}
                    className="border border-brand-200 rounded-lg overflow-hidden bg-white shadow-sm shadow-brand-500/5"
                  >
                        ...
                        >
                          <div className="flex items-start justify-between gap-4 mb-4">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                removeAdditionalProduct(category.id, product.id)
                              }
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-3">
                              {product.photos.map((photo, photoIndex) => (
                                <div
                                  key={photoIndex}
                                  className="relative group w-20 h-20 rounded-md overflow-hidden border border-slate-200 bg-slate-50"
                                >
                                  <Image
                                    src={photo.preview}
                                    alt="Product"
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  handleInputChange("categoryRemarks", e.target.value)
                }
