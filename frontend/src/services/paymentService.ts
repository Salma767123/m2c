...
class PaymentService {
  ...
  async createRazorpayOrder(amount: number, currency: string = 'INR'): Promise<RazorpayOrderResponse> {
    try {
  ...
  async verifyRazorpayPayment(
    ...
    try {
      const response = await axiosInstance.post('/payments/razorpay/verify', {
        razorpay_order_id,
        razorpay_payment_id,
        razorpay_signature
      });
