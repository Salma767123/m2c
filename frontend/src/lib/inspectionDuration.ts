// Duration math for vendor inspections — the single frontend source of truth for
// the report's active / paused / total breakdown and the "exceeded schedule" flag.
...
export function computeInspectionDurations(inspection: any, end?: Date | null): InspectionDurations {
  ...
  const endedAt = end ?? (inspection?.submittedAt
  ...
  if (!startedAt || !endedAt) {
    return { startedAt, endedAt: endedAt ?? null, activeMs: 0, pausedMs: 0, totalMs: 0, scheduledMs, exceeded: false }
  }
