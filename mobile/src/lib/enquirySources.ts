 ...
 *
 * A 1:1 port of frontend/src/lib/enquirySources.ts. Values are stable slugs
 * (never change them once live) so enquiries can be grouped and counted in
 * reports; labels are display-only and safe to reword. The storefront form, the
 * admin enquiry views, the source report and now this app all read the same set
 * — if the list changes on the web, change it here too.
 */
