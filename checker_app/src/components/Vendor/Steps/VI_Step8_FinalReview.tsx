...
// Gap left between the bottom of the remarks box and the keyboard.
const CLEARANCE = 24;

export default function VI_Step8_FinalReview({ verifications, meta, onMetaChange, onGoToStep }: Props) {
  const rejected = buildIssues(verifications);

  // The remarks box is the last field on the last step, so the keyboard opens
  // straight on top of it. This step can't scroll anything itself — it renders
  // inside the screen's ScrollView and has no ref to it — so instead it grows
  // run-out space underneath by exactly the overlap. That makes the box
  // reachable: the platform's own focused-input scrolling (and a flick from
  // the checker) can now bring it above the keyboard, which it cannot do when
  // the content ends flush at the bottom.
  const remarksRef = useRef<TextInput>(null);
  const keyboardTopRef = useRef(Number.POSITIVE_INFINITY);
  const [keyboardPad, setKeyboardPad] = useState(0);

  const syncPad = () => {
    const input: any = remarksRef.current;
    const keyboardTop = keyboardTopRef.current;
    if (!input || typeof input.measureInWindow !== 'function' || !Number.isFinite(keyboardTop)) return;
    input.measureInWindow((_x: number, y: number, _w: number, h: number) => {
      ...
      setKeyboardPad(overlap > 0 ? Math.round(overlap) : 0);
    });
  };
  ...
  useEffect(() => {
    ...
    const showSub = Keyboard.addListener(showEvt, (e: KeyboardEvent) => {
      ...
      setTimeout(syncPad, Platform.OS === 'android' ? 160 : 0);
    });
  ...
  // Focusing while the keyboard is already up fires no show event, so re-measure.
  const handleRemarksFocus = () => {
    if (Number.isFinite(keyboardTopRef.current)) {
      setTimeout(syncPad, Platform.OS === 'android' ? 160 : 60);
    }
  };
  ...
  return (
    <View style={{ rowGap: 28 }}>
      ...
      {/* Run-out room while the keyboard covers the remarks box. Collapses to
          nothing once the keyboard closes, so the step never carries dead space. */}
