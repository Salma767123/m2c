// Shared data definitions for the Product Inspection Form (checker app).
// Ported verbatim from the web QC portal:
//   frontend/src/components/Checker/Vendor/Steps/PI_data.ts
...
export function makeDefaultAdditionalEvidence(): Record<string, any[]> {
  ...
  ADDITIONAL_EVIDENCE_DEFS.forEach((d) => {
    out[d.id] = [];
  });
  return out;
}

// Remark code labels (shared by Packaging step + Review).
...
// Service type options (Step 1) — matches web PI_Step1_GeneralInfo SERVICE_TYPES.
...
// Inspection status options (Review step) — matches web INSPECTION_STATUS_OPTIONS.
export const INSPECTION_STATUS_OPTIONS = ['Approved', 'Rejected', 'On Hold', 'Re-Inspection'];
